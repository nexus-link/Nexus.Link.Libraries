...
namespace Nexus.Link.Libraries.Crud.UnitTests.Crud
{
    ...
    public abstract class TestICrudSearch<TId> : TestICrdBase<TestItemSort<TId>, TestItemSort<TId>, TId>
    {
        ...
        public async Task SearchOrderBy_Async(string value, bool orderByNumber, bool ascending)
        {
            ...
            if (orderByNumber)
            {
                page = await search.SearchOrderByAsync(new { Value = value }, new { IncreasingNumber = ascending }, 0, 10);
            }
            else
            {
                page = await search.SearchOrderByAsync(new { Value = value }, new { DecreasingString = ascending }, 0, 10);
            }
        ...
        public async Task SearchNestedOrderBy_Async()
        {
            ...
            page = await search.SearchOrderByAsync(new { Value = "Variant2" }, new { NumberModulo = true, IncreasingNumber = false}, 0, 10);
